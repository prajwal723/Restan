...
function Shopcontent({ img, desc, title, price, id, cart = [], setcart = () => {} }) {
  const Add = () => {
    const newCart = [...cart, id];
    setcart(newCart);
    localStorage.setItem("cartItem", JSON.stringify(newCart));
     console.log(`✅ Added id=${id} to cart:`, newCart);
    return newCart;
  };
    return <>
    <div className={`bg-white dark:bg-neutral-700/65  mx-3 mt-5 py-10 rounded-xl  justify-items-center-safe  'md:flex  `}>
        ...
        <div className="text-center space-y-1.5 ">
            ...
            <button className="border-2 border-[#826a45] dark:text-white hover:bg-[#826a45] hover:text-white rounded-3xl text-black/75 px-7 py-3 font-semibold text-[14px] " onClick={Add}>ADD TO CART</button>
        </div>
    </div>
    </>
}